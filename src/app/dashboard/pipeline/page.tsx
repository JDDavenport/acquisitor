...
import { getDeals, moveDealStage } from "@/app/actions/deals";
...
const stages = [
  ...
  { id: "diligence", name: "Diligence", color: "bg-orange-500", dotColor: "bg-orange-400", borderColor: "border-orange-500/30" },
  { id: "closing", name: "Closing", color: "bg-emerald-500", dotColor: "bg-emerald-400", borderColor: "border-emerald-500/30" },
  { id: "won", name: "Won", color: "bg-green-500", dotColor: "bg-green-400", borderColor: "border-green-500/30" },
];
  ...
  return (
    <Card className="border-navy-700/50 bg-navy-800/50 backdrop-blur-sm cursor-grab hover:bg-navy-700/50 hover:border-navy-600/50 transition-all duration-200 group">
      <CardContent className="p-4">
        <div className="flex items-start justify-between mb-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40 bg-navy-800 border-navy-700">
              ...
              {stages.map((s) => (
                <DropdownMenuItem key={s.id} className="text-navy-100 focus:bg-navy-700" onClick={() => handleStageChange(s.id)}>
                  Move to {s.name}
                </DropdownMenuItem>
              ))}
              <DropdownMenuSeparator className="bg-navy-700" />
        ...
        <p className="text-xs text-navy-400 mb-3">Lead #{deal.leadId?.substring(0, 8)}</p>

        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-sm">
              <DollarSign className="w-3.5 h-3.5 text-navy-500" />
              <span className="font-semibold text-white font-data">{formatCurrency(parseFloat(deal.value))}</span>
            </div>
          ...
          {deal.expectedCloseDate && (
            <div className="flex items-center gap-1.5 text-xs text-navy-400">
              <Calendar className="w-3 h-3" />Close: {formatDate(deal.expectedCloseDate)}
            </div>
          )}
        </div>
      </CardContent>
...
function PipelineColumn({ stage, deals }: { stage: typeof stages[0]; deals: any[] }) {
  const columnDeals = deals.filter((d) => d.stage === stage.id);
  const totalValue = columnDeals.reduce((s, d) => s + parseFloat(d.value), 0);
    ...
    <div className="flex flex-col shrink-0 w-72">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto min-h-[200px]">
        {columnDeals.length > 0 ? (
          ...
          <div className="text-center text-navy-500 text-sm py-8">No deals</div>
        )}
      </div>
    </div>
  );
...
export default function PipelinePage() {
  ...
  useEffect(() => {
    async function loadDeals() {
      if (session?.data?.user?.id) {
        try {
          const allDeals = await getDeals(session.data.user.id);
          setDeals(allDeals);
        } catch (error) {
  ...
  const totalWeightedValue = filteredDeals.reduce((s, d) => s + parseFloat(d.value) * (d.probability / 100), 0);
    ...
    <div className="space-y-6 h-full animate-slide-up">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Total Value", value: formatCurrency(filteredDeals.reduce((s, d) => s + parseFloat(d.value), 0)), color: "text-blue-400" },
          ...
          { label: "Closing Soon", value: filteredDeals.filter((d) => { 
      ...
      {/* Kanban */}
      {loading ? (
        <div className="text-center text-navy-400 py-12">Loading pipeline...</div>
      ) : (
